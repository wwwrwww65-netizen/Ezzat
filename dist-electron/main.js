...
var initDatabase = async () => {
	...
	if (count("invoices") === 0) [[
		...
		0,
		"معلقة",
		"مستخلص مرحلة التأسيس"
	]].forEach((i) => db.run("INSERT INTO invoices (id, client_id, client_name, project_id, date, amount, tax, total, paid_amount, status, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?)", i));
	if (count("income") === 0) db.run("INSERT INTO income (title, method, date, amount, status) VALUES ('دفعة أولى - فيلا النرجس', 'تحويل بنكي', '2023-12-05', 50000, 'مؤكد')");
	if (count("payments") === 0) db.run("INSERT INTO payments (id, entity_type, entity_id, project_id, amount, date, payment_method, reference, status, notes) VALUES ('PAY-001', 'client', 1, 1, 17250, '2023-12-02', 'تحويل بنكي', 'TRX9988', 'مؤكد', 'سداد فاتورة INV-001')");
	if (count("bonds") === 0) db.run("INSERT INTO bonds (id, type, entity_name, amount, date, payment_method, project, account, status, notes) VALUES ('BND-001', 'قبض', 'أحمد السديري', 50000, '2023-12-05', 'شيك', 'فيلا حي النرجس', 'البنك الأهلي', 'معتمد', 'دفعة مقدمة')");
	if (count("tasks") === 0) [[
		"مراجعة مخططات السباكة",
		1,
		1,
		"2023-12-15",
		"عالية",
		...
		"متوسطة",
		"لم تبدأ"
	]].forEach((t) => db.run("INSERT INTO tasks (title, project_id, assigned_to, due_date, priority, status) VALUES (?,?,?,?,?,?)", t));
	if (count("notifications") === 0) [[
		"تنبيه مخزون",
		"نقص في كمية الأسمنت البورتلاندي",
		"منذ ساعة",
		0,
		"warning"
	], [
		"موعد تسليم",
		"غداً موعد تسليم مرحلة العظم لفيلا النرجس",
		"منذ ساعتين",
		1,
		"info"
	]].forEach((n) => db.run("INSERT INTO notifications (title, message, time, read, type) VALUES (?,?,?,?,?)", n));
	if (count("activity_log") === 0) [[
		...
		"سارة خالد",
		"إصدار فاتورة جديدة #INV-2023-002",
		"2023-12-10 10:30",
		"192.168.1.5"
	]].forEach((l) => db.run("INSERT INTO activity_log (user, action, time, ip) VALUES (?,?,?,?)", l));
	if (count("documents") === 0) [
		[
			"عقد مشروع فيلا النرجس",
			"pdf",
			"2.5 MB",
			"2023-10-01",
			"العقود والاتفاقيات",
	...
	].forEach((d) => db.run("INSERT INTO documents (name, type, size, date, folder, project, uploader) VALUES (?,?,?,?,?,?,?)", d));
	if (count("users") === 0) db.run("INSERT INTO users (username, password, role, name) VALUES (?, ?, ?, ?)", [
		"admin",
		"admin123",
		"admin",
		"مدير النظام"
	]);
	if (count("job_roles") === 0) for (const [name, permissions, description] of [
		[
			"مدير النظام",
			"{\"all\":true}",
		...
		[
			...
			"إدارة المالية والتقارير"
		],
		[
			"مهندس",
			"{\"projects\":true,\"boq\":true}",
			"إدارة المشاريع والجداول"
		],
		[
			"مشرف",
			"{\"projects\":true}",
			"متابعة المشاريع ميدانياً"
		],
		[
			"مدير موارد بشرية",
			"{\"hr\":true}",
		...
		[
			"عميل",
			"{}",
			"دخول لوحة العميل فقط"
		]
	]) db.run("INSERT OR IGNORE INTO job_roles (name, permissions, description) VALUES (?,?,?)", [
		name,
		permissions,
		description
	]);
	if (count("penalty_rules") === 0) for (const [type, reason, amount, is_system] of [
		[
			"absence",
			"غياب بدون عذر",
			0,
	...
	]) db.run("INSERT INTO penalty_rules (type, reason, amount, is_system) VALUES (?,?,?,?)", [
		type,
		reason,
		amount,
		is_system
	...
	if (count("categories") === 0) [
		[
			"الأسمنت",
			"CEM",
			"جميع أنواع الأسمنت والبورتلاندي",
			"#1e3a8a"
		],
		[
			"البلك",
			"BLK",
			"بلك أسمني، بركاني، معزول",
			"#3b82f6"
		],
		[
			"الحديد",
			"STEL",
			"حديد تسليح بمختلف المقاسات",
			"#ef4444"
		],
		[
			"الكهرباء",
		...
		[
			"الحفر والأساسات",
			"EXC",
			"أعمال الدفان والحفر والعزل",
			"#8b4513"
		],
		[
			"العظم والخرسانة",
			"CONC",
			"الخرسانة الجاهزة والرمل والكنكري",
			"#64748b"
		],
		[
			"التشطيبات والأرضيات",
			"FIN",
			"البلاط، الدهانات، والأسقف",
			"#10b981"
		],
		[
			"السباكة",
			"PLUM",
			"الأنابيب والمواسير والأطقم",
			"#0ea5e9"
		],
		[
			"الأبواب والنوافذ",
			"DOOR",
			"الأبواب الخشبية وشتر الألمنيوم",
			"#8b5cf6"
		],
		[
			"الأعمال التشغيلية",
			"OPS",
			"أعمال الحفر، النقل، والخدمات",
			"#f59e0b"
		]
	].forEach((c) => db.run("INSERT INTO categories (name, code, description, color) VALUES (?,?,?,?)", c));
	...
	].forEach((m) => db.run("INSERT INTO materials_catalog (name, category_id, unit, buy_price, sell_price) VALUES (?,?,?,?,?)", m));
...
function createWindow() {
	...
	win.webContents.on("context-menu", (event, params) => {
		...
		} else if (params.selectionText) menu.append(new MenuItem({
			label: "نسخ (Copy)",
			role: "copy"
		}));
